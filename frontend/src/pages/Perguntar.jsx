...
export default function Perguntar() {
  ...
  // segundo plano, e a tela fica utilizável durante ela.
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const recarregar = async () => {
        ...
        if (!vivo) return
        setIndice(i => ({ ...i, trechos, bm25: trechos.length ? construirBM25(trechos) : null }))
        return trechos
      }
  ...
  const carregarAnteriores = useCallback(async () => {
    ...
    try {
      ...
      if (!chats?.length) return setAnteriores([])
  ...
  useEffect(() => { carregarAnteriores() }, [carregarAnteriores])
  ...
  async function enviar(e, texto) {
    ...
    try {
      ...
      const resumoDoAcervo = (acervo || []).map(c => ({
        id: c.id,
      ...
      const { escolhidos, conversasBuscadas, conversasUsadas } = buscar({
        trechos: indice.trechos, pergunta, plano, vetorPergunta: vetor, bm25: indice.bm25,
      })
